...
class DbScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          DbBloc(UserDatabase(), CardStorage() /*UserDatabase()*/)
                      ...
                      itemBuilder: (context, i) {
                        return GestureDetector(
                          ...
                          child: Dismissible(
                            ...
                            child:
                                cards['${users[i].id.toString()}_${users[i].name}'] !=
                                        null
                                    ? Card(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Text(cards[
                                                    '${users[i].id.toString()}_${users[i].name}'] ??
                                                'error'),
                                          ],
                                        ),
                                      )
                ...
                onPressed: () async {
                  BlocProvider.of<DbBloc>(context).add(InsertUserEvent(
                      userHelper: UserHelper(
                          name: 'test', surname: 'testov', cardNumber: 2202)));
                },
              );
            },
          )),
