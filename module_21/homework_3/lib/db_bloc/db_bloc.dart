...
class DbBloc extends Bloc<DbEvent, DbState> {
  ...
  DbBloc(UserDatabase userDatabase, CardStorage cardStorage)
  ...
  _getAllUsersEvent(
      GetAllUsersEvent event, Emitter<DbState> emit) /*async*/ async {
//рабочий код для future списка юзеров
    ...
    await _cardStorage.readAllSecureData().then(
        (value) => emit((state as AllUsersState).copyWith(cardNumbers: value)));

    await emit.forEach(_userDatabase.usersStream, onData: (List<User> users) {
...
//print('last id ${(state as AllUsersState).users[(state as AllUsersState).users.length-1].id}');
    ...
    await _cardStorage
        .addCard(key, event.userHelper.cardNumber.toString())
        .whenComplete(() => _cardStorage.readAllSecureData().then((value) =>
            emit((state as AllUsersState).copyWith(cardNumbers: value))));
