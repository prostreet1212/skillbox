...
class ImageBloc extends Bloc<ImageEvent, ImageState> {
  ImageBloc() : super(ImageState(Future.value([]))) {
  ...
  _onLoadImageFromMemory(LoadImageFromMemoryEvent event, Emitter<ImageState> emit)async {
    ...
    emit(ImageState(Future.value(s)));
    return s;
  }

  _onLoadImageFromUrl(LoadImageFromUrlEvent event, Emitter<ImageState> emit)async {
    Uint8List? imageBytes;
    //if(Uri.parse(event.url).isAbsolute){
      final appDir = await getApplicationDocumentsDirectory();
      var response = await Dio().get<List<int>>(event.url,
          options: Options(responseType: ResponseType.bytes));
      String nameImage = event.url.substring(
